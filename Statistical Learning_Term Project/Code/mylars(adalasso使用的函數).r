...
mylars<-function (X, y, k = 10,use.Gram=TRUE,normalize=TRUE,intercept=TRUE) 
{
  ...
  globalfit<-glmnet(x,y,family="binomial",standardize=normalize,type.gaussian=type,intercept=intercept)
  lambda<-globalfit$lambda
  residmat <- matrix(0, length(lambda), k)
  for (i in seq(k)) {
    omit <- all.folds[[i]]
    fit <- glmnet(x[-omit, ,drop=FALSE], y[-omit],type.gaussian=type,standardize=normalize,family="binomial",intercept=intercept)
